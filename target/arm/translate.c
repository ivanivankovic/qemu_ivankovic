...
{
    int state = get_started_bb();
    // state 0 means that logging is stopped and we don't need to call anything
    if (state == 0) {
        return;
    // state 2 means that we want to clear all data stored currently and then step to state 0
    } else if (state == 2){
        afl_prev_loc = 0;
        ...
        clear_bb_enter();
//        FILE *f = fopen("map.txt", "wb");
//        fwrite(dummy, sizeof(unsigned char), sizeof(dummy), f);
//        fclose(f);
    }
